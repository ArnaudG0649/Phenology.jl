...
function Plot_Both_Pheno_Dates(date_vecs_EB, date_vecs_BB, CPO; sample_EB=nothing, sample_BB=nothing, labelvec=nothing, colors=nothing, rightlegend=false)
    fig = Figure(size=(700, 600))
    ...
    pltvec = Plot_Pheno_Dates_ax!(fig[3:4, 1:2], date_vecs_EB, CPO,
        sample_=sample_EB,
...
"""
function Phenology.Plot_Pheno_Dates_EB_BB(date_vecEB::Vector{Date}, date_vecBB::Vector{Date}, CPO; sample_EB=nothing, sample_BB=nothing, station_name="", YearCut=nothing, save_file=nothing, loaded_data=nothing, display_legend=true)
    ...
    ScaleDateEB(date_) = ScaleDate(date_, CPO, false)
    ScaleDateBB(date_) = ScaleDate(date_, CPO, true)

    NDSCPO_EB = ScaleDateEB.(date_vecEB) #NDSCPO : Number of dates since chilling period onset
    NDSCPO_BB = ScaleDateBB.(date_vecBB)

    #We concatanate the dates of each series to have the max and the min NDSCPO
    Conc_date_vecs_EB = isnothing(sample_EB) ? date_vecEB : ([date_vecEB; reduce(vcat, sample_EB)])
    ...
    NDSCPO_month = ScaleDateEB.(Dates_month) #And their NDSCPO

    if isnothing(loaded_data)
        NDSCPO_inf = maximum(NDSCPO_month[NDSCPO_month.<=minimum(ScaleDateEB.(Conc_date_vecs_EB))]) #The optimal NDSCPO_month to minimise the true NDSCPO.
        NDSCPO_sup = minimum(NDSCPO_month[NDSCPO_month.>=maximum(ScaleDateEB.(Conc_date_vecs_BB))]) #The optimal NDSCPO_month to maximise the true NDSCPO.
    else
        raw_NDSCPO_inf = minimum([ScaleDateEB.(Conc_date_vecs_EB); EB_dict["NDSCPO"]; EB_dict["minimum"]])
        NDSCPO_inf = maximum(NDSCPO_month[NDSCPO_month.<=raw_NDSCPO_inf])
        raw_NDSCPO_sup = maximum([ScaleDateEB.(Conc_date_vecs_BB); BB_dict["NDSCPO"]; BB_dict["maximum"]])
    ...
    for (sample_, colors, SD_func, Dict_) in zip([sample_EB, sample_BB], [[("#e5ca20", 0.2), ("#e5ca20", 0.5)], [("#009bff", 0.2), ("#009bff", 0.5)]], [ScaleDateEB, ScaleDateBB], Dictionnaries)
    ...
    if !isnothing(loaded_data)
        for (dict_, color_) in zip([EB_dict, BB_dict], ["brown4", "magenta"])
            ...
            push!(pltvec, band!(ax, years, dict_["q25"], dict_["q75"], color=(color_, 0.25)))
            push!(pltvec, lines!(ax, years, dict_["NDSCPO"], color=color_,))
        end
    end

    #NDSCPO plots
    push!(pltvec, lines!(ax, year.(date_vecEB + min(Month(2), Year(1) - Month(CPO[1]) - Day(CPO[2]))), NDSCPO_EB, color="#ff6600"))
    push!(pltvec, lines!(ax, year.(date_vecBB), NDSCPO_BB, color="green"))



    if !isnothing(save_file)
        Dictionnaries[1]["years"] = year.(date_vecEB + min(Month(2), Year(1) - Month(CPO[1]) - Day(CPO[2])))
        Dictionnaries[1]["NDSCPO"] = NDSCPO_EB
    ...
    if display_legend
        if isnothing(loaded_data)
            ...
            Legend(fig[3:4, 5], pltvec[[1, 2, 13, 7]], ["Min-Max interval of predictions\non simulated temperatures",
            ...
            Legend(fig[1:2, 5], pltvec[[3, 4, 14, 12]], ["Min-Max interval of predictions\non simulated temperatures",
                    "Quartile interval of predictions\non simulated temperatures",
...
"""
function Phenology.Plot_Pheno_Dates_EB_BB(date_vecsEB, date_vecsBB, CPO;
    EB_colors=nothing,
    BB_colors=nothing,
    EB_label=nothing,
    BB_label=nothing,
    ...
    NDSCPO_vecs_EB = [ScaleDateEB.(date_vecEB) for date_vecEB in date_vecsEB]
    NDSCPO_vecs_BB = [ScaleDateBB.(date_vecBB) for date_vecBB in date_vecsBB]

    #We concatanate the dates of each series to have the max and the min NDSCPO
    Conc_date_vecs_EB = reduce(vcat, date_vecsEB)
    ...
    #NDSCPO plots
    EB = true
    for (date_vecs, NDSCPO_vecs, colors) in zip([date_vecsEB, date_vecsBB], [NDSCPO_vecs_EB, NDSCPO_vecs_BB], [EB_colors, BB_colors])
        if isnothing(colors)
            for (date_vec, NDSCPO_date_vec, i) in zip(date_vecs, NDSCPO_vecs, eachindex(NDSCPO_vecs))
                push!(pltvec, lines!(ax, year.(date_vec), NDSCPO_date_vec, linestyle=i ∈ dashindexes ? :dash : :solid))
            end
            ...
            if breakpoints == true
                for (date_vec, NDSCPO_date_vec, i, color_) in zip(date_vecs, NDSCPO_vecs, eachindex(NDSCPO_vecs), colors)
                    push!(pltvec, scatterlines!(ax, year.(date_vec), NDSCPO_date_vec, linestyle=i ∈ dashindexes ? :dash : :solid, color=color_, marker=EB ? :vline : :star4))
                end
            else
                for (date_vec, NDSCPO_date_vec, i, color_) in zip(date_vecs, NDSCPO_vecs, eachindex(NDSCPO_vecs), colors)
                    push!(pltvec, lines!(ax, year.(date_vec), NDSCPO_date_vec, linestyle=i ∈ dashindexes ? :dash : :solid, color=color_))
    ...
    #Label
    isnothing(EB_label) ? nothing : Legend(fig[3:4, 5], pltvec[1:length(date_vecsEB)], EB_label, "Endodormancy break", framevisible=false)
    isnothing(BB_label) ? nothing : Legend(fig[1:2, 5], pltvec[length(date_vecsEB)+1:end], BB_label, "Budburst", framevisible=false)
...
"""
function Phenology.Plot_Freeze_Risk(TN_vecs, dates_vecs_TN, date_vecsBB;
    CPO=(8, 1),
    colors=nothing,
    label=nothing,
    size=(800, 400),
...
"""
function Phenology.Plot_Freeze_Risk_Bar(TN_vec, dates_vec_TN, date_vecBB;
    CPO=(8, 1),
    color=nothing,
    label=nothing,
    size=(800, 400),
...
function Phenology.Plot_Freeze_Risk_Bar(df::DataFrame, date_vecBB;
    CPO=(8, 1),
    color=nothing,
    label=nothing,
    size=(800, 400),
    threshold=-2)
    return (Phenology.Plot_Freeze_Risk_Bar(df.TN, df.DATE, date_vecBB;
        CPO=CPO,
        color=color,
        label=label,
        size=size,
...
"""
function Phenology.Plot_Freeze_Risk_sample(TN_vecs, Date_vec, date_vecsBB; threshold=-2., PeriodOfInterest=Month(3), CPO=(10, 30), size_fig=(1300, 400))
...
"""
function Phenology.PlotHistogram(date_vecEB::Vector{Date}, date_vecBB::Vector{Date}, CPO, year; sample_EB=nothing, sample_BB=nothing, station_name="", LineHeight=0.15, stationlegend=false, horline=true, PlotQuantile=false)
    inyear(date_) = Date(year - 1, CPO[1], CPO[2]) + Month(1) .<= date_ .< Date(year, CPO[1], CPO[2]) + Month(1)
    EB, BB = date_vecEB[findfirst(inyear, date_vecEB)], date_vecBB[findfirst(inyear, date_vecBB)]
    ...
    NDSCPO_EB = ScaleDateEB(EB)
    NDSCPO_BB = ScaleDateBB(BB)

    sample_EB_year = reduce(vcat, sample_EB)
    sample_EB_year = sample_EB_year[inyear.(sample_EB_year)]
    sample_BB_year = reduce(vcat, sample_BB)
    sample_BB_year = sample_BB_year[inyear.(sample_BB_year)]

    #We concatanate the dates of each series to have the max and the min NDSCPO
    Conc_date_vecs_EB = [[EB]; sample_EB_year]
    ...
    SDEB = ScaleDateEB.(sample_EB_year)
    SDBB = ScaleDateBB.(sample_BB_year)

    push!(pltvec, hist!(ax, SDEB, color="#e5ca20", strokewidth=1, strokecolor=:black, normalization=:pdf))
    push!(pltvec, lines!(ax, [ScaleDateEB(EB), ScaleDateEB(EB)], [0., LineHeight], color="#ff6600"))
    ...
    if PlotQuantile
        push!(pltvec, lines!(ax, [quantile(SDEB, 0.25), quantile(SDEB, 0.25)], [0., LineHeight], color="black"))
        push!(pltvec, lines!(ax, [quantile(SDEB, 0.75), quantile(SDEB, 0.75)], [0., LineHeight], color="black"))
